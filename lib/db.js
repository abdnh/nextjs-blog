...
class DB {
    ...
    async read() {
        ...
        await this.db.read()
        this.db.data ||= { posts: [], users: [], comments: [] }
    ...
    async createUser(username, password, admin = false) {
        ...
        if (existingUser) {
            throw Error(`يوجد بالفعل مستخدم يحمل اسم "${username}"`);
        }
        const salt = await bcrypt.genSalt()
        const hash = await bcrypt.hash(password, salt)
        const user = { username: username, password: hash, admin: admin }
    ...
    async createComment(date, postID, username, content) {
        await this.read()
        const comment = {
            date, postID, username, content
        }
        this.db.data.comments.push(comment)
        await this.db.write()
        return comment
    }
...
const db = new DB();
export default db;
// db.createUser("admin", "admin", true)
